...
class ThumbnailUpdater implements Runnable {
	...
	public void resetTileValues() {
		...
		//@tag zest.sequence.bug -author="Del Myers" -date="enCA:26/03/07" : this may cause memory problems, but it should solve the no more handles error.
		hTiles = Math.min(hTiles, 24);
		vTiles = Math.min(vTiles, 24);
